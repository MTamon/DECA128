...
class SRenderY(nn.Module):
    ...
    def forward(
        self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type="point", background=None
    ):
        """
        -- Texture Rendering
        ...
        if background is not None:
            ...
            albedo_images = albedo_images * alpha_images + background * (1.0 - alpha_images)
        else:
    ...
    def world2uv(self, vertices):
        ...
        uv_vertices = self.uv_rasterizer(
            self.uvcoords.expand(batch_size, -1, -1), self.uvfaces.expand(batch_size, -1, -1), face_vertices
        )[:, :3]
        return uv_vertices
